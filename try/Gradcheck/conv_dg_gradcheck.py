...
class Conv(torch.nn.Module):
    ...
    Convolution layer.
    """
    def __init__(self, K, padding=0, stride=1, b=None):
        super(Conv, self).__init__()
        self.padding = padding
        self.stride = stride
        self.b = torch.nn.Parameter(torch.zeros([1,1,1,1], dtype=torch.double)) if b is None else b
    ...
    def forward(self, X):
        ...
        Z = torch.zeros([bx,cx,h_out,w_out], dtype=torch.double)
        for b in range(0, bx):
            for c in range(0, cx):
