import torch
from conv_dg_gradcheck import Conv
import numpy as np
import sys
    ...
    def init_params(self, Ks=None, W=None, T=None):  # Define trainable parameters here, should be nn.Parameter or with requires_grad=True
        if Ks is None:    # Initialize default Conv filters if nothing is passed via arguments
            for conv_layer in self.conv_layers:
                self.Ks.append(torch.nn.Parameter(torch.ones(conv_layer['filter_shape'], dtype=torch.double)))
        else:
        ...
        self.W = torch.nn.Parameter(torch.zeros((self.num_labels, self.crf_input_dim), dtype=torch.double)) if W is None else W
        self.T = torch.nn.Parameter(torch.zeros((self.num_labels, self.num_labels), dtype=torch.double)) if T is None else T
...
class CRFautograd(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        return grad_Ws, grad_Ts, torch.ones(X.shape, dtype=torch.double), None  # TODO: Replace 3rd with d(loss)/d(X), where X is CRF input
    ...
    def forward_messages(X, W, T):
        ...
        for i in range(n):
            f_msgs[0, i] = torch.exp(torch.dot(X[0], W[i]).double())
        for i in range(1, m):
            for a in range(n):
                product = torch.exp(torch.dot(X[i], W[a]).double())
                tmp = torch.zeros(n)
                for b in range(n):
    ...
    def backward_messages(X, W, T):
        n = len(T)  # 26 letters
        m = len(X)  # word length
        b_msgs = torch.zeros((m, n), dtype=torch.double)
        for i in range(n):
            b_msgs[-1, i] = torch.exp(torch.dot(X[-1], W[i]).double())
    ...
    def compute_grad_W(X, y, W, T):
        n = len(T)
        m = len(X)
        grad_W = torch.zeros(W.shape, dtype=torch.double)
    ...
    def compute_grad_T(X, y, W, T):
        n = len(T)
        m = len(X)
        grad_T = torch.zeros(T.shape, dtype=torch.double)
        f_msgs = CRFautograd.forward_messages(X, W, T)
        b_msgs = CRFautograd.backward_messages(X, W, T)
        Z = torch.sum(f_msgs[-1])
    ...
    def crf_obj_prime(W, T, X_data, y_data):
        n = len(T)
        dim = W.shape[1]
        grad_Ws = torch.zeros((n, dim), dtype=torch.double)
        grad_Ts = torch.zeros((n, n), dtype=torch.double)
