...
class CRF(torch.nn.Module):
    ...
    def init_params(self, Ks=None, W=None, T=None):  # Define trainable parameters here, should be nn.Parameter or with requires_grad=True
        if Ks is None:    # Initialize default Conv filters if nothing is passed via arguments
            for conv_layer in self.conv_layers:
                self.Ks.append(torch.nn.Parameter(torch.ones(conv_layer['filter_shape'])))
        else:
    ...
    def forward(self, X, labels):
        ...
        """
        features = self.get_conv_feats(X)
        log_prob = CRFautograd.apply(features, labels, self.W, self.T)
        return log_prob
...
class CRFautograd(torch.autograd.Function):
    @staticmethod
    def forward(ctx, X, y, W, T):
        l, r = CRFautograd.fwdBwdMsgs(X, y, W, T)
        ctx.save_for_backward(X, y, W, T, l, r)
        return CRFautograd.compute_log_probability(X, y, W, T, l, r)

    @staticmethod
    def backward(ctx, grad_out):
        # print('Bwd called')
        X, y, W, T, l, r = ctx.saved_tensors
        g_W, g_T, g_X = CRFautograd.compute_gradient(X, y, W, T, l, r)
        g_W = grad_out * g_W
        g_T = grad_out * g_T
        g_X = grad_out * g_X
        return g_X, None, g_W, g_T

    @staticmethod
    def fwdBwdMsgs(X, y, W, T):
        num_ex = len(X)
        num_label = len(T)  # 26
    ...
    @staticmethod
    def compute_gradient(X, y, W, T, l, r):
        num_ex = len(X)
        num_features = len(W)
        num_label = len(T)  # 26
        ...
        g_X = torch.zeros((num_ex, max_word_length, num_features))
        for ex in range(num_ex):
            word_label = torch.nonzero(y[ex])[:, 1]
            word = X[ex, :len(word_label)]
            num_letter = len(word_label)
