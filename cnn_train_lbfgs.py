...
if args.path is not None:
	...
	PATH = "./{}_lbfgs.pt".format(args.model)
...
def main():
	...
	for epoch in range(NUM_EPOCHS):
		...
		for i_batch, sample in enumerate(train_loader, 0):
			...
			def closure():
				...
				print('Loss at epoch {}, batch {}: {}'.format(epoch + 1, i_batch, tr_loss.item()))
				tr_loss.backward()
				del outputs
				return tr_loss
	...
	save_accuracies(letter_training_accuracies, letter_test_accuracies, "letter", args.model, "lbfgs")
	save_accuracies(word_training_accuracies, word_test_accuracies, "word", args.model, "lbfgs")
